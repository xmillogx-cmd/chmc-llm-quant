#!/usr/bin/env python3
"""
make_article_figures.py — фигуры для статьи (CPU)
==================================================

Генерирует три PNG в results_v6/article/figures/:
  1. fig_versions.png   — эволюция PPL ratio v1→v6 (лог-шкала, уровень GPTQ = 1.0);
  2. fig_ppl.png        — PPL ratio к исходной модели: CHMC v6 vs GPTQ по трём моделям;
  3. fig_bottleneck.png — «бутылочное горлышко»: доля энергии top-3 ПК (объединённое
                          центрированное облако [pre;post]) и средний попарный косинус
                          pre-токенов по блокам, три модели.

Методика fig_bottleneck.png совпадает с Рисунком 1 / Таблицей 2 статьи:
детерминированная подвыборка 512 токенов (равномерный шаг, как в project_3d.py),
PCA — numpy SVD центрированного объединения [pre;post] на блок.
...
def make_fig_versions():
    ...
    ax.text(5.45, 1.03, "уровень GPTQ (ratio = 1.0)", ha="right", va="bottom",
    ...
    ax.set_ylabel("PPL ratio к GPTQ (лог-шкала)")
    ax.set_title("CHMC v1 → v6 на SmolLM-135M: путь от коллапса до паритета", fontsize=11.5)
...
def make_fig_ppl():
    # PPL ratio к исходной модели (FP16). См. Таблицу 4 статьи.
    labels = ["SmolLM-135M", "Qwen2.5-0.5B*", "TinyLlama-1.1B"]
    chmc = [1.1825, 1.174, 1.059]      # SmolLM — середина диапазона 1.179–1.186
    gptq = [1.176, 1.141, 1.081]

    x = np.arange(len(labels))
    w = 0.36
    fig, ax = plt.subplots(figsize=(7.2, 4.4), dpi=150)
    b1 = ax.bar(x - w / 2, chmc, w, color="#e8871a", label="CHMC v6 (среднее по сидам)")
    ...
    for bars, vals in ((b1, chmc), (b2, gptq)):
        for b, v in zip(bars, vals):
            ax.annotate(f"{v:.3f}", xy=(b.get_x() + b.get_width() / 2, v),
                        ...
                        ha="center", fontsize=9.5)
    ax.axhline(1.0, color="#5a6472", ls="--", lw=1.2)
    ax.text(2.45, 1.003, "исходная модель (FP16)", ha="right", va="bottom",
            fontsize=9, color="#5a6472")
    ax.set_xticks(x, labels)
    ax.set_ylim(1.0, 1.235)
    ax.set_ylabel("PPL ratio к исходной модели")
    ax.legend(fontsize=9, loc="upper right")
    ax.grid(axis="y", alpha=0.25)
    ax.set_title("CHMC v6 против GPTQ при равном бит-бюджете 4.2875 BPW\n"
                 "(* Qwen — одиночный прогон; остальные — среднее по 3–4 сидам)", fontsize=11.5)
    fig.tight_layout()
...
def make_fig_bottleneck():
    ...
    for model, _ in MODELS:
        ...
        for bi in range(n_blocks):
            ...
            # top-3 доля энергии: центрированное объединение [pre;post]
            X = np.vstack([Pp, Qq])
            Xc = X - X.mean(axis=0)
            _, S, _ = np.linalg.svd(Xc, full_matrices=False)
            top3.append(float((S[:3] ** 2).sum() / (S ** 2).sum()))

            # средний попарный косинус pre-токенов (сырые векторы)
    ...
    ax1.set_xlabel("decoder-блок")
    ax1.set_ylabel("доля энергии top-3 ПК, %")
    ax1.set_title("Концентрация: top-3 ПК объединённого облака [pre;post]", fontsize=11)
    ax1.grid(alpha=0.25)
    ax1.legend(fontsize=9)

    ax2.set_xlabel("decoder-блок")
    ax2.set_ylabel("средний попарный косинус pre-токенов")
    ax2.set_title("Анизотропия: средний косинус внутри блока", fontsize=11)
    ax2.grid(alpha=0.25)

    fig.suptitle("Бутылочное горлышко: после первых 1–2 блоков почти вся сеть лежит "
                 "в низкоразмерном конусе (позиция пика различается по моделям)", fontsize=11.5)
