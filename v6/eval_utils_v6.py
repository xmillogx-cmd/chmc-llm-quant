#!/usr/bin/env python3
"""
eval_utils_v6.py — v6-local copy of the unified evaluation utilities
====================================================================

This is a verbatim copy of v5/eval_utils_v5.py, moved into the current
version (v6) so that any fix can be made here without touching the shared
v5 module. Behavior is identical to eval_utils_v5:

  1. Deterministic PPL with proper overlap masking (BUG-1 fix)
  2. Model-specific token cache keyed by vocab size
  3. Consistent API — all v6 scripts import ONLY from this module
...
if DEVICE == "cuda":
    print(f"[eval_v6] GPU detected: {torch.cuda.get_device_name(0)}")
else:
    print("[eval_v6] CPU mode (no CUDA)")
...
def load_wikitext_eval(tokenizer: PreTrainedTokenizer, n_tokens: int = 12000) -> torch.Tensor:
    ...
    try:
        ...
        import pandas as pd
        print(f"[eval_v6] Downloading wikitext-2-raw-v1 test split...")
        ...
        if len(encoded) < n_tokens:
            ...
            try:
                ...
                print("[eval_v6] Test split too short, loading train split...")
        ...
        print(f"[eval_v6] Cached ({vocab_size} vocab): {len(encoded)} tokens")
        return encoded[:n_tokens]
    except Exception as e:
        # Try training split as fallback
        try:
            from datasets import load_dataset
            print(f"[eval_v6] Test split failed ({e}), trying train+test...")
...
def load_calib_text(tokenizer: PreTrainedTokenizer, n_tokens: int = 2048) -> str:
    ...
    try:
        from datasets import load_dataset
        print("[eval_v6] Loading wikitext-2-raw-v1 TRAIN split for calibration...")
        ds_train = load_dataset("wikitext", "wikitext-2-raw-v1", split="train")
        ...
        print(f"[eval_v6] datasets failed ({e}), trying hf_hub_download train parquet...")
        from huggingface_hub import hf_hub_download
        import pandas as pd
        parquet_path = hf_hub_download(
            repo_id="Salesforce/wikitext",
    ...
    with open(calib_cache, "w", encoding="utf-8") as f:
        f.write(text)
    print(f"[eval_v6] Calib text cached (train split, {vocab_size} vocab): {len(text)} chars")
    return text
    ...
    for name in layer_names:
        ...
        if mod is not None and isinstance(mod, nn.Linear):
            ...
            def hook(m, inp, out, store=captured):
                ...
                # Offload to CPU: holding all 252 layers' activations on GPU
                # (~4 GB for 3B) alongside the FP32 model OOMs a 16 GB card.
                # They are moved back to GPU per-layer during compression.
                store.append(x.flatten(0, 1).cpu())
