...
# Import quantize_symmetric_per_channel from chmc_v4
from chmc_v4 import quantize_symmetric_per_channel


def select_best_lowrank_method(W, X, rank):
    W_lr, err = weighted_svd_compress(W, X, rank)
    return "weighted_svd", W_lr, err
...
orig_weights = {}
for name in layers:
    orig_weights[name] = get_weight(mdl, name).detach().clone()
